...
<head>
    ...
    <title>Airing Schedule - AniStream</title>
...
<body class="bg-[#0b0b0b] min-h-screen flex">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 lg:static lg:w-64 bg-[#111] border-r border-[#00f3ff]/20 transition-transform duration-300 z-50 lg:z-40 flex flex-col lg:h-screen lg:sticky lg:top-0">
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-2">
            ...
            <a href="schedule.php" class="block px-4 py-2 rounded bg-gray-800 text-[#00f3ff] transition font-bold border border-[#00f3ff]/30">📅 Schedule</a>
    ...
    <main class="flex-1 flex flex-col relative min-w-0 h-screen overflow-y-auto">
        <header class="p-4 border-b border-gray-800 flex items-center bg-[#0f0f0f] z-20 shrink-0">
            ...
            <h1 class="text-xl font-bold neon-text">SCHEDULE</h1>
        </header>

        <div class="flex-1 p-6">
            <h2 class="text-2xl font-bold mb-6 uppercase tracking-wider text-[#00f3ff]">Currently Airing</h2>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            const query = `query { Page(perPage: 30) { media(sort: POPULARITY_DESC, status: RELEASING, type: ANIME, isAdult: false) { id title { english romaji } coverImage { extraLarge } format } } }`;
            ...
            data.data.Page.media.forEach(anime => {
                grid.innerHTML += `<div class="cursor-pointer transition-all duration-300 rounded-lg overflow-hidden poster-hover relative bg-gray-900 group" onclick="openDetails(${anime.id})">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black via-black/80 to-transparent pt-12">
                        <p class="text-sm font-semibold truncate text-gray-100">${anime.title.english || anime.title.romaji}</p>
                    </div>
                </div>`;
            });
        });
        ...
        async function openDetails(id) {
            ...
            content.innerHTML = '<div class="p-12 text-center text-[#00f3ff] font-bold">Loading...</div>';
