...
<head>
    ...
    <title>Popular TV - Zentrix Stream</title>
...
<body class="bg-[#0b0b0b] text-white min-h-screen flex">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 lg:static lg:w-64 bg-[#111] border-r border-gray-800 transition-transform duration-300 z-50 lg:z-40 flex flex-col lg:h-screen lg:sticky lg:top-0">
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-2">
            ...
            <a href="popular-tv.php" class="block px-4 py-2 rounded bg-red-600/10 text-red-500 transition font-bold border border-red-600/30 border-l-4">📺 Popular TV</a>
    ...
    <main class="flex-1 flex flex-col relative min-w-0 h-screen overflow-y-auto">
        <header class="p-4 lg:p-6 border-b border-gray-800 flex items-center bg-[#0f0f0f] z-20 shrink-0">
            ...
            <h1 class="text-xl font-bold red-text uppercase flex items-center gap-2">
                <span class="w-1 h-6 bg-red-600 rounded hidden lg:block"></span> Popular TV Shows
            </h1>
        </header>
    ...
    <script>
        ...
        async function loadContent() {
            try {
                const res = await fetch(`../api.php?route=tv/popular`);
                ...
                grid.innerHTML = data.results.map(item => `
                    <div class="cursor-pointer transition-all duration-300 rounded-lg overflow-hidden poster-hover relative bg-gray-900 shadow-lg" onclick="window.location.href='watch.php?id=${item.id}&type=tv'">
                        <div class="aspect-[2/3]">
                            <img src="${item.poster_path ? 'https://image.tmdb.org/t/p/w500'+item.poster_path : 'https://via.placeholder.com/500x750/111/ff0000?text=No+Poster'}" class="w-full h-full object-cover" alt="${item.name}" loading="lazy">
                        </div>
                        <div class="p-3 bg-gradient-to-t from-black via-black/80 to-transparent absolute bottom-0 w-full pt-12">
                            <p class="text-xs font-bold truncate drop-shadow-md">${item.name}</p>
                            <span class="text-[10px] text-white font-bold bg-red-600 px-1.5 py-0.5 rounded mt-1 inline-block">★ ${item.vote_average ? item.vote_average.toFixed(1) : 'N/A'}</span>
                        </div>
                    </div>
                `).join('');
            } catch (e) {
